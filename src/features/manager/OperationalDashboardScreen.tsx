...
export const OperationalDashboardScreen = () => {
  ...
  // === [FE_FN_001] Truy vấn danh sách các loại xe đang kích hoạt ===
  // - API Endpoint: GET /operation/vehicle-types?activeOnly=true
  // - Chức năng: Lấy thông tin các loại xe (Car, Motorbike, v.v.) để lấy iconUrl hoặc hiển thị nhãn tương ứng.
  ...
  // === [FE_FN_002] Truy vấn danh sách lịch sử phiên đỗ xe phân trang ===
  // - API Endpoint: GET /operation/parking-sessions/all
  // - Chức năng: Lấy danh sách lịch sử đỗ xe chi tiết (ngày giờ vào, ra, biển số, loại xe, tổng phí) phục vụ cho General History Table.
  ...
  // === [FE_FN_003] Hàm xử lý xuất báo cáo lịch sử đỗ xe ra tệp CSV ===
  // - API Endpoint: GET /operation/parking-sessions/export
  // - Chức năng: Đóng gói và tải xuống tệp CSV chứa danh sách lịch sử đỗ xe theo khoảng thời gian đã chọn.
  ...
  // === [FE_FN_004] Truy vấn trạng thái hoạt động của các cổng kiểm soát (Gates Status) ===
  // - API Endpoint: GET /infrastructure/gates
  // - Chức năng: Lấy thông tin các cổng trong bãi xe để theo dõi trạng thái đóng/mở, tình trạng kết nối.
  ...
  // === [FE_FN_005] Truy vấn dữ liệu tổng quan vận hành theo ngày (Check-ins, Check-outs, Live Capacity) ===
  // - API Endpoint: GET /api/v1/finance/dashboard/operational
  // - Chức năng: Lấy số liệu phục vụ cho KPI check-ins/outs trong ngày và thống kê sức chứa thời gian thực (Live Capacity).
  ...
  // === [FE_FN_006] Truy vấn bản đồ phân bổ các Zone đỗ xe ===
  // - API Endpoint: GET /infrastructure/zones/map
  // - Chức năng: Lấy cấu trúc phân bổ các khu vực (Zone) đỗ xe trong bãi phục vụ hiển thị thông tin sức chứa theo Zone.
  ...
  // === [FE_FN_007] Truy vấn danh sách nhân viên tuần tra đang hoạt động (Active Patrols) ===
  // - API Endpoint: GET /identity/work-sessions/active/patrol
  // - Chức năng: Lấy danh sách các nhân viên bảo vệ/tuần tra đang trực ca để hỗ trợ quản lý vận hành.
  ...
  // === [FE_FN_008] Truy vấn lưu lượng xe vào/ra theo khung giờ (Hourly Traffic Flow) ===
  // - API Endpoint: GET /api/v1/finance/dashboard/hourly-flow
  // - Chức năng: Lấy dữ liệu lượng xe In/Out theo từng khung giờ trong ngày để dựng biểu đồ lưu lượng Hourly Traffic Flow.
  ...
  // === [FE_FN_009] Memo hóa dữ liệu Sức chứa từ Dashboard Data ===
  const liveData = useMemo(() => {
    if (dashboardData?.liveData) return dashboardData.liveData;
    return { vehicleStats: [], checkIns: 0, checkOuts: 0 };
  }, [dashboardData]);

  // === [FE_FN_010] Tính toán giờ cao điểm và lượng xe lớn nhất (Traffic Peak Hour) ===
