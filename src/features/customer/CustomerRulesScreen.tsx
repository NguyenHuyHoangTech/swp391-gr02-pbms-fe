 ...
 * @Author: Thái Tân Phú
 * @Date: 28/07/2026
 * @Description: Màn hình Quy định Bãi đỗ xe (Customer Rules) - Hiển thị nội quy, quy định lấy từ thông tin cấu hình tòa nhà.
 * @Dependencies: 
 * - React, antd
 * - axiosClient, react-query
 */
...
export const CustomerRulesScreen = () => {
  ...
  // - Mã giả: Gọi API `/public/building-profile` để lấy danh sách nội quy (trường `rules`).
  // ==========================================
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {buildingProfile?.rules ? (
          <section className="relative mb-12">
            {/* ========================================== */}
            {/* [RENDER]: HIỆU ỨNG NỀN ÁNH SÁNG MỜ (AMBIENT BACKGROUND) */}
            {/* ========================================== */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {buildingProfile.rules.split('\n').filter((r:string) => r.trim()).map((item: string, index: number) => (
                    <div key={index} className="group relative bg-white/70 backdrop-blur-md rounded-2xl p-8 border border-white/60 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden">
                        {/* ========================================== */}
                        {/* [RENDER]: SỐ THỨ TỰ IN CHÌM (WATERMARK) */}
                        {/* ========================================== */}
