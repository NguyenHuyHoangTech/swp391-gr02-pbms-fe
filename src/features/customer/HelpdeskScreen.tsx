...
/**
 * =========================================================================================
 * CHI TIẾT VÒNG ĐỜI VÀ KIẾN TRÚC XỬ LÝ CỦA HelpdeskScreen.tsx (KÈM MINH CHỨNG CODE)
 * (Trình bày chi tiết luồng hoạt động ánh xạ trực tiếp vào các dòng code trong file này)
 * =========================================================================================
 * 
 * BƯỚC 1: QUẢN LÝ ĐỊNH TUYẾN ĐA NỀN TẢNG (RESPONSIVE ROUTING & HISTORY API)
 * - Minh chứng 1: Sự tồn tại của 2 hàm `renderMobileView()` và `renderDesktopView()`.
 *   Bằng chứng cho thấy Component này tự động thích ứng với kích thước màn hình mà 
 *   không cần thiết kế riêng 2 trang độc lập.
 * - Minh chứng 2: Khối lệnh `window.addEventListener('popstate', handlePopState)`.
 *   Đây là thủ thuật nâng cao can thiệp vào nút Back vật lý của điện thoại. Khi khách 
 *   hàng bấm Back, hệ thống sẽ chỉ đóng Form chứ không văng ra khỏi ứng dụng Web.
 * 
 * BƯỚC 2: TỰ ĐỘNG FETCH DỮ LIỆU THỜI GIAN THỰC (REAL-TIME POLLING)
 * - Minh chứng: Khối lệnh `useQuery` gọi API `/incident/incidents` đi kèm với 
 *   cấu hình `refetchInterval: 5000`.
 * - Bằng chứng cho thấy Component không cần Socket vẫn có thể tự động "hút" dữ liệu 
 *   từ Server 5 giây một lần để cập nhật trạng thái mới nhất cho Khách hàng.
 * 
 * BƯỚC 3: QUẢN LÝ TRẠNG THÁI LUÂN PHIÊN (STATE TOGGLING)
 * - Minh chứng: Việc sử dụng đồng thời 3 State (`selectedTicket`, `selectedCategory`, 
 *   `queueFilter`) chứng tỏ màn hình này gánh vác vai trò như một "Mini SPA" (Single 
 *   Page Application) thu nhỏ. 
 * - Tùy thuộc vào các State này mà giao diện sẽ luân chuyển mượt mà giữa "Danh sách", 
 *   "Form tạo mới", và "Xem chi tiết" mà không cần tải lại trang.
 * =========================================================================================
 */
